...
const I18N = {
    ...
    strings: {
        ...
        infoHowTitle: { en: 'About This App', 'zh-TW': '關於網站', de: 'So verwenden Sie diese App', ja: 'このアプリの使い方' },
        infoHowDesc: { en: `Welcome to the ultimate lightweight Fate RPG app, designed to help tabletop players and Game Masters streamline their storytelling without digital clutter. Whether you need a responsive Fate Core online character sheet for a long-running campaign or an intuitive Fate Accelerated character builder to fast-track your session zero, our platform keeps your Aspects, Stress, and Fate Points organized in real time. Featuring a flexible Fate custom skill pyramid tool, you can easily tailor homebrew skills and custom settings to fit any adventure, whether you're playing at an in-person table or running a virtual session on desktop or mobile.`, 'zh-TW': '歡迎使用這款專為桌上角色扮演遊戲（TTRPG）玩家與主持人（GM）打造的輕量化 Fate RPG 應用程式，讓您擺脫繁瑣的數位干擾，專注於流暢的說故事體驗。無論您需要的是適用於長期戰役的響應式 Fate Core 線上角色卡，還是能加速零號集會（Session 0）進程的直覺式 Fate Accelerated 角色建立工具，我們的平台都能即時幫您整理好相位（Aspects）、壓力（Stress）與命運點數（Fate Points）。搭配靈活的 Fate 自訂技能金字塔工具，不論是在實體跑團或線上遠端連線，您都能輕鬆調整自訂技能與世界觀設定，適應任何冒險情節，完美支援電腦與行動裝置。', de: 'Willkommen zur ultimativen leichtgewichtigen Fate-RPG-App, die speziell für Spielende und Spielleitungen entwickelt wurde, um Ihr Storytelling ohne digitalen Ballast zu optimieren. Egal, ob Sie einen reaktionsschnellen Online-Charakterbogen für Fate Core für eine langfristige Kampagne oder einen intuitiven Charakter-Baukasten für Fate Accelerated für ein schnelles Session-Zero-Setup benötigen – unsere Plattform hält Ihre Aspekte, Stress und Fate-Punkte in Echtzeit übersichtlich. Dank unseres flexiblen Tools für benutzerdefinierte Fate-Skill-Pyramiden können Sie Homebrew-Fertigkeiten und individuelle Settings mühelos an jedes Abenteuer anpassen, egal ob Sie am heimischen Spieltisch sitzen oder eine digitale Runde auf dem PC oder Smartphone leiten.', ja: 'TRPGプレイヤーとゲームマスターのために設計された、シンプルで使いやすい軽量Fate RPGアプリへようこそ！デジタル面での煩わしさを解消し、物語の進行に集中できる環境を提供します。長期キャンペーンに最適な応答性の高い Fate Core オンラインキャラクターシート や、セッション0をスピーディーに進められる直感的な Fate Accelerated キャラクタービルダー など、アスペクト、ストレス、フェイトポイントをリアルタイムで整理できます。さらに、柔軟な Fate カスタムスキルピラミッドツール を活用すれば、ハウスルールや独自のWeb設定にも簡単に対応可能。対面でのセッションはもちろん、デスクトップやモバイルでのオンラインセッションにも最適です。' },
        infoVersionTitle: { en: 'Version', 'zh-TW': '版本', de: 'Version', ja: 'バージョン' },
        infoVersion: { en: 'Fate Create v1.4 Added custom portraits and also randomized skills.', 'zh-TW': 'Fate Create v1.4 新增自訂頭像，並將技能隨機化。', de: 'Fate Create v1.4 Benutzerdefinierte Porträts hinzugefügt und Fähigkeiten randomisiert.', ja: 'Fate Create v1.4 カスタムポートレートを追加し、スキルもランダム化しました。' },
